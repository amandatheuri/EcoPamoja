...
class ActionChallengeManager extends StatelessWidget {
  const ActionChallengeManager({super.key});
    ...
    return StreamBuilder<List<ChallengeModel>>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          return const Center(child: Text('No action challenges found.'));
        }

        final actionChallenges = snapshot.data!
            .where((c) => c.type == ChallengeType.action)
            .toList();
